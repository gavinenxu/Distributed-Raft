...
func (cfg *config) makeClient(to []int) *Clerk {
	...
	ck := NewClerk(random_handles(ends))
...
func (cfg *config) StartServer(i int) {
	...
	ends := make([]*rpc.ClientEnd, cfg.n)
	for j := 0; j < cfg.n; j++ {
		ends[j] = cfg.net.AddClientEnd(cfg.endnames[i][j])
	...
	cfg.servers[i] = NewShardController(ends, i, cfg.saved[i])

	kvsvc := rpc.NewService(cfg.servers[i])
	rfsvc := rpc.NewService(cfg.servers[i].rf)
	srv := rpc.NewServer(i)
